...
// AtomicCounterを使って高速にシステム全体で一意なIDを生成する
func dispenseID(ctx context.Context) (string, error) {
	return strconv.FormatInt(atomicCounter.Inc(), 10), nil
}
...
func retrieveCompetitionBulk(ctx context.Context, tenantDB dbOrTx, ids []string) ([]CompetitionRow, error) {
	if len(ids) == 0 {
	...
	if err := tenantDB.SelectContext(ctx, &cs, query, args...); err != nil {
		return nil, fmt.Errorf("error Select competition: ids=%v, %w", ids, err)
	}
	return cs, nil
}
...
func playerHandler(c echo.Context) error {
	...
	query += ") AND player_id = ? ORDER BY row_num DESC"
	args := make([]interface{}, 0, len(cs)+2)
	...
	if err := tenantDB.SelectContext(ctx, &pss, query, args...); err != nil {
		return fmt.Errorf("error Select player_score: tenantID=%d, competitionIDs=%v, playerID=%s, %w", v.tenantID, cs, p.ID, err)
	}

	psds := make([]PlayerScoreDetail, 0, len(pss))
	psids := make([]string, 0, len(pss))

	for _, ps := range pss {
		psids = append(psids, ps.CompetitionID)
	}

	comps, err := retrieveCompetitionBulk(ctx, tenantDB, psids)
	if err != nil {
		return fmt.Errorf("error retrieveCompetitionBulk: %w", err)
	}
	compMap := make(map[string]CompetitionRow, len(comps))
	for _, comp := range comps {
		compMap[comp.ID] = comp
	}
	for _, ps := range pss {
		psds = append(psds, PlayerScoreDetail{
			CompetitionTitle: compMap[ps.CompetitionID].Title,
			Score:            ps.Score,
		})
	}
...
func competitionRankingHandler(c echo.Context) error {
	...
	go func(playerId string, tenantId int64, competitionId string) {
		if _, err := adminDB.ExecContext(
			ctx,
			"INSERT INTO visit_history (player_id, tenant_id, competition_id, created_at) VALUES (?, ?, ?, ?)",
			playerId, tenantId, competitionId, time.Now().Unix(),
		); err != nil {
	...
	players, err := retrievePlayerBulk(ctx, tenantDB, retrievePlayerRequestIds)
	if err != nil {
		return fmt.Errorf("error retrievePlayerBulk: %w", err)
	}
	...
	for _, ps := range pss {
		ranks = append(ranks, CompetitionRank{
			...
			PlayerDisplayName: playersMap[ps.PlayerID].DisplayName,
			RowNum:            ps.RowNum,
		})
	}
